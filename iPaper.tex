...
Signals in annual late-wood $\delta^{13}$C and \gls{SPI} 
...
respectively (Fig. \ref{fig:cohe}). The wavelet coherency is based on
the theory that signals can be decomposed over mother wavelets
\begin{align}
...
wavelet \citep{Cazelles2008} which is well localized in scales and has
high frequency resolution. Under these definitions, the wavelet
coherency is computed following three steps. First, wavelet transform
...
\begin{align}
W_{\delta,\gamma}(p,\tau)=W_{\delta}(p,\tau)W_{\gamma}(f,\tau)
\end{align}

Finally, the wavelet coherence is computed as the cross-spectrum
normalized by sectrum of each signal:
\begin{align}
R_{\delta, \gamma}(f,\tau)=\frac{\left \| \left \langle W_{\delta,\gamma}(f,\tau) \right \rangle \right \|}{\left \| \left \langle W_{\delta,\delta}(f,\tau) \right \rangle \right \|^{1/2}\left \| \left \langle W_{\gamma,\gamma}(f,\tau) \right \rangle \right \|^{1/2}}
\end{align}

where $\left \langle \right \rangle$ is an smooting operator and
$0 \le R_{\delta, \gamma}(f,\tau) \le 1$
